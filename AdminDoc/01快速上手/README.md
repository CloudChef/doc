...
* 多云平台的统一管理，统一使用体验
* 面向整体应用的端到端自动化部署，包括云主机、存储网络、中间件数据库等应用软件等等
* 整合ITSM和ITOM，定义和发布标准的工单服务，提供完整的IT解决方案
* 实时云上资源监控，多渠道告警通知
* 云上成本分析和优化，按需分账
* 全自动流水线，打通开发工具链，实现DevOps
...
1.  点击添加，输入模板名称，例如CentOS 7.3 on Aliyun，选择之前配置的阿里云云平台并做如下配置：
  ...
  - 镜像中已启用SSH：勾选该选项，表示模板中已经启用和配置SSH，可以选择通过SSH的方式安装监控代理和自动化代理；默认SSH端口为22，如果模板中修改了SSH的端口，可输入模板中修改的SSH端口号
...
1.  点击添加，输入模板名称：Windows 2012 on Aliyun，选择2.1节配置的阿里云云平台并做如下配置：
  ...
  - 镜像中已启用WinRM：勾选该选项，表示模板中已经启用和配置WinRM，可以选择通过WinRM的方式安装监控代理和自动化代理。
