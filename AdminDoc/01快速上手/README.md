...
　　SmartCMP智能云管理平台，致力于解决企业多云资源和企业应用的管理问题，为您和您的企业带来：

  + 多云统一管理：纳管复杂异构云，整合多厂商云能力，为您提供从IaaS到DaaS的综合云服务。

  + 面向业务部门的自服务：提供多种开箱即用的应用组件，您可以自定义各种自助服务。

  + 跨云服务编排和自动化：基于国际标准TOSCA模型，实现跨混合环境、多云场景的服务编排能力，为您提供面向应用的端到端自动化部署。

  + 完善的人员、组织和权限控制：细颗粒度的角色和权限管理，并提供多层级业务组，助您控制资源与服务的访问。

  + 策略驱动资源分配：为资源进行归类，并提供策略编辑，您可以灵活的分配您的基础资源。

  + 完善的成本分析和优化：通过自定义成本核算规则，快速洞察多云环境成本，结合资源使用率，帮助您优化工作负载部署，合理化配置。

  + 资源全生命周期管理：贯穿运维管理全过程，从资源交付到资源回收，有效控制闲置资源。

  + 灵活定制的流程：通过基于Web界面的设计器轻松定制流程，为您的IT提供丰富、灵活，同时遵循管控要求的服务定义方式。
...
　　4. 创建虚拟机模板和计算规格，供申请部署云资源时候使用。

　　5. 填加IP池，定义可用的IP地址范围（可选）。

　　6. 定义业务组，关联用户、资源池，配置审批流程、部署规范等。

　　7. 申请部署云资源。

　　8. 自助运维，管理云资源，查看监控信息，进行自助运维操作。

　　

　　目前各个云平台能够支持的云资源部署类型如下图所示：
...
　　虚拟机模板是平台对操作系统的抽象和标准化。平台提供了内置的通用操作系统，您也可以新增，并为各个操作系统关联云平台的虚拟机模板（例如vSphere）或操作系统镜（例如阿里云），供您和用户在部署虚拟机的时候进行选择，或由平台根据您的配置自动进行选择。
...
1.  选择 基础设施 - 虚拟机模板 ，进入虚拟机模板列表界面。该页面默认有1个Windows虚拟机模板，Windows Server 2012 R2

2.  下面以Windows Server 2012 R2为例介绍如何关联云平台模板。点击 Windows 2012 R2 ，进入虚拟机模板基本信息标签页，该页面显示操作系统名称、描述、以及系统类型（LInux或Windows)
     *  若勾选 允许在申请时选择公有云平台中所有的镜像，则允许用户在申请部署云资源时候，选择公有云平台上所有的镜像。
...
　　如您需要关联公有云平台的规格（以阿里云为例），请按以下步骤操作：
...
　　至此，您已经可以通过平台成功申请部署云资源，并进行管理和运维操作。
