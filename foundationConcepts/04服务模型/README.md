...
丰富的组件库:开箱即用的组件资源。SmartCMP云管平台内置大量常用的组件资源，包括IaaS（不同云平台的云主机、网络、存储等等）、PaaS（RDS）、容器（Kubernetes、Docker）、软件(MySQL、Oracle、Nginx等等)、代理（监控代理），不同类型资源在统一的界面进行创建和管理，采用树形结构的分组目录，支持自定义组件分组，支持二次编辑、快速复制、导入、导出组件。

> 「Note」详细的创建和管理步骤请参考：[组件库](http://CMP-PUBLIC-IP/help/AdminDoc/05服务建模/组件库.html#组件库)
...
> 「Note」自定义组件的详细步骤请参考：[创建组件](http://CMP-PUBLIC-IP/help/AdminDoc/05服务建模/组件库.html#创建组件)
