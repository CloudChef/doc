...
+ 登录路径：所有租户都可以使用该租户下的用户直接访问登录页面（骞云工程师会为您配置好URL）例如：http://cmp.smartcmp.online:1688/#/login
+ 如果需要指明具体租户的ID，例如使用系统管理进行登录的时候，可访问骞云工程师会为您配置好URL后添上后缀“ ?tenant=（租户ID）”例如：http://cmp.smartcmp.online:1688/#/login?tenant=（租户ID）
